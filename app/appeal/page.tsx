...
export default function Page() {
  ...
  async function sendRequest() {
    if (!message.trim()) return alert(t("appealEmpty"));
    const result = await execute("/api/requests", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ name: user?.firstName || "Employee", email: user?.email || "", type: "appeal", message }),
    });
    if (!result) return;
    setSuccessMsg(t("appealSuccess"));
    setMessage("");
  }
    ...
    <div dir={isRTL ? "rtl" : "ltr"} className="min-h-screen bg-white flex items-center justify-center p-5">
      <div className="w-full max-w-lg bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
        <div className="flex justify-between items-center px-6 py-4 bg-[#030405]">
          <h1 className="text-xl font-bold text-white">{t("appealTitle")}</h1>
          <LangToggle dark />
        </div>
        <div className="p-6">
        {error      && <p className="text-red-500 text-sm mb-2">{error}</p>}
        {successMsg && <p className="text-green-600 text-sm mb-2">{successMsg}</p>}
        <textarea placeholder={t("appealPlaceholder")} value={message} onChange={(e) => setMessage(e.target.value)}
        ...
        <button onClick={sendRequest} disabled={submitting} className="w-full mt-4 py-3 rounded-full text-white font-semibold bg-gradient-to-r from-[#F33615] to-[#ff6b4a] disabled:opacity-60">
          {submitting ? t("sending") : t("sendAppeal")}
        </button>
